...
def intentar_reconstruir(cam_id: int):
    ...
    if len(frags) < n_total:
        return  # Faltan fragmentos

    # Ensamblar en orden
    jpg_bytes = b"".join(frags[i] for i in range(n_total))
...
#  Lee byte a byte para detectar tanto JSON (texto)
#  como paquetes binarios de imagen
# ════════════════════════════════════════════════
_raw_buf  = bytearray()   # buffer acumulado de bytes crudos
_text_buf = ""            # buffer de texto para líneas JSON

# Tamaños de paquetes binarios (deben coincidir con el C3)
FRAG_SIZE      = 26
SIZEOF_INICIO  = 8   # tipo(1) + cam_id(1) + size_total(4) + n_frags(2)
SIZEOF_FRAG    = 32  # tipo(1) + cam_id(1) + frag_idx(2) + frag_total(2) + datos(26)
SIZEOF_FIN     = 2   # tipo(1) + cam_id(1)

def procesar_byte(b: int):
    """Procesa un byte del serial: decide si es texto o binario."""
    global _text_buf

    # Si el byte es ASCII imprimible o newline, acumular como texto
    if b == 0x0A:  # newline → procesar línea
    ...
    # Byte no-ASCII → inicio de paquete binario
    # Delegar al parser binario
    _raw_buf.append(b)
    intentar_parsear_binario()

def intentar_parsear_binario():
    """Intenta parsear el inicio de _raw_buf como paquete binario."""
    global _raw_buf
    ...
    if tipo == PKT_FOTO_INICIO:
        if len(_raw_buf) < SIZEOF_INICIO:
            return  # Esperar más bytes
        cam_id     = _raw_buf[1]
        size_total = struct.unpack_from("<I", _raw_buf, 2)[0]
        n_frags    = struct.unpack_from("<H", _raw_buf, 6)[0]
        ...
        _raw_buf = _raw_buf[SIZEOF_INICIO:]

    elif tipo == PKT_FOTO_FRAG:
        if len(_raw_buf) < SIZEOF_FRAG:
            return
        cam_id    = _raw_buf[1]
        frag_idx  = struct.unpack_from("<H", _raw_buf, 2)[0]
        frag_tot  = struct.unpack_from("<H", _raw_buf, 4)[0]
        datos     = bytes(_raw_buf[6:6 + FRAG_SIZE])
        if cam_id not in _img_fragments:
            _img_fragments[cam_id] = {}
        _img_fragments[cam_id][frag_idx] = datos
        _raw_buf = _raw_buf[SIZEOF_FRAG:]
        intentar_reconstruir(cam_id)

    elif tipo == PKT_FOTO_FIN:
        if len(_raw_buf) < SIZEOF_FIN:
            return
        cam_id = _raw_buf[1]
        log_msg(f"CAM{cam_id} fin recibido")
        _raw_buf = _raw_buf[SIZEOF_FIN:]
        intentar_reconstruir(cam_id)

    elif tipo == PKT_TELEMETRY:
        # Paquete de telemetría binario (struct Telemetry del C3)
        # sizeof: 1 + 6*4 + 4*1 = 29 bytes
        SIZEOF_TEL = 29
        if len(_raw_buf) < SIZEOF_TEL:
            return
        # Parsear struct: tipo(1) + temp,pres,alt,yaw,pitch,roll (6 floats) + sys,gyro,accel,mag (4 uint8)
        try:
            _, temp, pres, alt, yaw, pitch, roll, sys_, gyro, accel, mag = \
                struct.unpack_from("<Bffffffbbbb", _raw_buf, 0)
            # Actualizar UI igual que con JSON
            vals = [temp, pres, alt, yaw, pitch, roll]
            for k, val in zip(KEYS, vals):
                data[k].append(val)
            for key, _, _, _ in DISPLAY:
                idx = KEYS.index(key)
                value_vars[key].set(f"{vals[idx]:.2f}")
            cal_vars["sys"].set(str(sys_))
            cal_vars["gyro"].set(str(gyro))
            cal_vars["accel"].set(str(accel))
            cal_vars["mag"].set(str(mag))
            log.append(vals)
            guardar_fila(vals)
            paquetes[0] += 1
            pk_var.set(f"PAQUETES: {paquetes[0]}")
            ...
            pass
        _raw_buf = _raw_buf[SIZEOF_TEL:]

    else:
        # Byte desconocido, descartar y seguir
        _raw_buf = _raw_buf[1:]
