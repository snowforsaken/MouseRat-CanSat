...
# ════════════════════════════════════════════════
MAX_LEN = 100
KEYS    = ["temp", "pres", "alt", "yaw", "pitch", "roll", "vel"]
data    = {k: deque([0.0] * MAX_LEN, maxlen=MAX_LEN) for k in KEYS}

# Para calcular velocidad vertical entre muestras
_last_alt  = None
_last_time = None
_vel_window = deque(maxlen=5)   # media móvil sobre 5 muestras para suavizar ruido de altitud
log     = []

session_start = datetime.now().strftime("%Y%m%d_%H%M%S")
csv_path      = ruta(f"cansat_log_{session_start}.csv")
txt_path      = ruta(f"cansat_datos_{session_start}.txt")

with open(csv_path, "w", newline="") as f:
    csv.writer(f).writerow(["timestamp", "temp", "pres", "alt", "yaw", "pitch", "roll", "vel_ms"])

with open(txt_path, "w") as f:
    f.write("Hora,Fecha,Temperatura,Presion,Altura,GiroscopioX,GiroscopioY,Velocidad_ms\n")

def guardar_fila(vals):
    with open(csv_path, "a", newline="") as f:
        csv.writer(f).writerow([datetime.now().isoformat()] + vals)

def guardar_fila_txt(temp, pres, alt, pitch, roll, vel):
    now = datetime.now()
    with open(txt_path, "a") as f:
        f.write(f"{now.strftime('%H:%M:%S')},{now.strftime('%d/%m/%Y')},{temp:.2f},{pres:.2f},{alt:.2f},{pitch:.2f},{roll:.2f},{vel:.2f}\n")
...
sec_title(img_pnl, "MAPA DE PROFUNDIDAD", C["accent"])
...
def procesar_profundidad(buf: bytes) -> bytes | None:
    """
    Genera mapa de profundidad monocular a partir de un solo JPEG.
    Método: gradiente Sobel + desenfoque gaussiano para suavizar,
    luego colormap JET — objetos con bordes fuertes aparecen 'cerca'
    (rojo) y zonas homogéneas 'lejos' (azul).
    """
    try:
        ...
        # Suavizado previo para reducir ruido de compresión JPEG
        gray = cv2.GaussianBlur(gray, (5, 5), 0)

        # Gradiente Sobel en X e Y
        sx = cv2.Sobel(gray, cv2.CV_64F, 1, 0, ksize=3)
        sy = cv2.Sobel(gray, cv2.CV_64F, 0, 1, ksize=3)
        magnitude = np.sqrt(sx**2 + sy**2)

        # Suavizado del mapa de gradiente para que se vea continuo
        magnitude = cv2.GaussianBlur(magnitude, (15, 15), 0)

        # Normalizar a 0-255
        depth = cv2.normalize(magnitude, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)

        # Colormap JET: azul=lejos, rojo=cerca (igual que el mapa de disparidad)
        color_map = cv2.applyColorMap(depth, cv2.COLORMAP_JET)

        ok, encoded = cv2.imencode('.jpg', color_map, [cv2.IMWRITE_JPEG_QUALITY, 75])
        return encoded.tobytes() if ok else None
    except Exception as e:
        log_msg(f"Error en mapa de profundidad: {e}")
        return None
...
GRAPH_KEYS   = ["temp", "pres", "alt", "vel", "pitch", "roll"]
GRAPH_COLORS = [C["accent2"], C["accent"], C["green"], C["red"], C["yellow"], C["yellow"]]
GRAPH_LABELS = ["Temp (°C)", "Pres (hPa)", "Alt (m)", "Vel (m/s)", "Pitch (°)", "Roll (°)"]

fig, axs = plt.subplots(len(GRAPH_KEYS), 1, figsize=(4, 7))
...
def procesar_json(line: str):
    ...
    try:
        ...
        # Velocidad vertical: Δalt / Δt entre muestras consecutivas
        global _last_alt, _last_time
        now = time.time()
        if _last_alt is not None and _last_time is not None:
            dt = now - _last_time
            vel = (alt - _last_alt) / dt if dt > 0.05 else 0.0
            # Suavizado simple para evitar spikes de ruido del BMP
            vel = max(-150.0, min(150.0, vel))
        else:
            vel = 0.0
        _last_alt  = alt
        _last_time = now

        vals = [temp, pres, alt, yaw, pitch, roll, vel]
        ...
        log.append(vals[:6])
        guardar_fila(vals[:6] + [vel])
        guardar_fila_txt(temp, pres, alt, pitch, roll, vel)
...
def intentar_reconstruir(cam_id: int):
    ...
    # Reconstruir con lo que haya — fragmentos faltantes se rellenan con ceros
    if recibidos < n_total * 0.3:
        return
    ...
    # Generar mapa de profundidad monocular con la imagen recibida
    log_msg("Generando mapa de profundidad monocular...")
    resultado = procesar_profundidad(jpg_bytes)
    if resultado:
        root.after(0, lambda r=resultado: mostrar_imagen(r, 0))
        log_msg(f"Mapa de profundidad listo: {len(resultado)} bytes")
    else:
        ...
        # ── PKT_FOTO_FIN ──────────────────────────
            ...
            if cam_id in _img_totales:
                ...
                if resultado:
                    ...
                    root.after(0, lambda b=jpg_bytes, c=cam_id: mostrar_imagen(b, c))
                _img_fragments.pop(cam_id, None)
                _img_totales.pop(cam_id, None)
...
log_msg(f"Log CSV: {csv_path}")
log_msg(f"Log TXT: {txt_path}")
